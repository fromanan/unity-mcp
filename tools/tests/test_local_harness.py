...
class TestDiscoverEditorPerOS:
    ...
    def test_windows_resolves_hub_layout(self):
        ...
        binary = str(PureWindowsPath(r"C:\Program Files") / "Unity" / "Hub" / "Editor" / version / "Editor" / "Unity.exe")
        exists, is_exec, list_dir = _fake_fs({binary})
        spec = resolve_editor_binary(
            version,
            platform="win32",
    ...
    def test_linux_resolves_hub_layout(self):
        version = "6000.0.75f1"
        env = {"HOME": "/home/dev"}
        binary = str(PurePosixPath("/home/dev") / "Unity" / "Hub" / "Editor" / version / "Editor" / "Unity")
        exists, is_exec, list_dir = _fake_fs({binary})
        spec = resolve_editor_binary(
            version,
            platform="linux",
...
class TestDiscoverEditorMonkeypatchedPlatform:
    ...
    def test_default_platform_linux(self, monkeypatch):
        ...
        env = {"HOME": "/home/ci"}
        binary = str(PurePosixPath("/home/ci") / "Unity" / "Hub" / "Editor" / version / "Editor" / "Unity")
        exists, is_exec, list_dir = _fake_fs({binary})
        spec = resolve_editor_binary(
            version,
            environ=env,
...
class TestSecondaryInstallPath:
    ...
    def test_secondary_root_used_for_candidate(self):
        ...
        binary = str(PurePosixPath(secondary) / version / "Unity.app/Contents/MacOS/Unity")
        exists, is_exec, list_dir = _fake_fs({binary})
